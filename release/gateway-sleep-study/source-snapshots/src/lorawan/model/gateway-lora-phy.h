...
class GatewayLoraPhy : public LoraPhy
{
  public:
    ...
    /**
     * Enable or disable PHY sleep mode for the gateway.
     *
     * While in sleep mode, incoming receptions must be ignored by the PHY
     * implementation and any ongoing receptions are aborted.
     *
     * @param sleeping True to put the PHY to sleep, false to wake it up.
     */
    void SetSleepMode(bool sleeping);

    /**
     * Check whether the PHY is currently in sleep mode.
     *
  ...
  protected:
    ...
    /**
     * Trace source fired when a packet starts arriving while the gateway PHY is asleep.
     */
    TracedCallback<Ptr<const Packet>, uint32_t> m_noReceptionBecauseSleep;

    /**
     * Trace source fired when an ongoing reception is aborted because the gateway PHY
     * transitions to sleep.
     */
    TracedCallback<Ptr<const Packet>, uint32_t> m_receptionAbortedBySleep;

    bool m_isTransmitting; //!< Flag indicating whether a transmission is going on
    bool m_isSleeping;     //!< Flag indicating whether PHY sleep mode is enabled
