                                   ...
                                   uint32_t frequencyHz)
{
    ...
    if (IsSleepMode())
    {
        NS_LOG_INFO("Dropping packet reception because gateway PHY is sleeping");
        if (m_device)
        {
            m_noReceptionBecauseSleep(packet, m_device->GetNode()->GetId());
        }
        else
        {
            m_noReceptionBecauseSleep(packet, 0);
        }
        return;
    }
...
SimpleGatewayLoraPhy::EndReceive(Ptr<Packet> packet, Ptr<LoraInterferenceHelper::Event> event)
{
    ...
    Ptr<SimpleGatewayLoraPhy::ReceptionPath> matchingPath;
    ...
    // Sleep or transmit transitions can invalidate a reception before its
    // completion callback executes. Ignore stale completions in that case.
    if (matchingPath == nullptr)
    {
        NS_LOG_INFO("Ignoring stale EndReceive event after reception invalidation");
        return;
    }
