...
GatewayLoraPhy::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::GatewayLoraPhy")
            ...
            .AddTraceSource("LostPacketBecauseSleep",
                            "Trace source indicating a packet could not be received because the "
                            "gateway PHY was asleep when reception started",
                            MakeTraceSourceAccessor(&GatewayLoraPhy::m_noReceptionBecauseSleep),
                            "ns3::Packet::TracedCallback")
            .AddTraceSource("LostPacketBecauseSleepAbort",
                            "Trace source indicating a packet reception was aborted because the "
                            "gateway PHY transitioned to sleep",
                            MakeTraceSourceAccessor(&GatewayLoraPhy::m_receptionAbortedBySleep),
                            "ns3::Packet::TracedCallback")
...
GatewayLoraPhy::ResetReceptionPaths()
{
    ...
    for (auto& path : m_receptionPaths)
    {
        if (!path->IsAvailable())
        {
            Ptr<LoraInterferenceHelper::Event> event = path->GetEvent();
            if (event)
            {
                Ptr<const Packet> packet = event->GetPacket();
                if (m_device)
                {
                    m_receptionAbortedBySleep(packet, m_device->GetNode()->GetId());
                }
                else
                {
                    m_receptionAbortedBySleep(packet, 0);
                }
            }
            EventId endReceive = path->GetEndReceive();
            if (endReceive.IsPending())
            {
                Simulator::Cancel(endReceive);
            }
            path->Free();
        }
    }
    m_occupiedReceptionPaths = 0;

    m_receptionPaths.clear();
}

void
GatewayLoraPhy::SetSleepMode(bool sleeping)
{
    NS_LOG_FUNCTION(this << sleeping);

    if (m_isSleeping == sleeping)
    {
        return;
    }
    ...
    // Abort and invalidate any ongoing receptions when entering sleep mode.
    for (auto& path : m_receptionPaths)
    {
        if (!path->IsAvailable())
        {
    ...
    m_occupiedReceptionPaths = 0;
    m_interference.ClearAllEvents();
}

bool
GatewayLoraPhy::IsSleepMode() const
{
    return m_isSleeping;
}
